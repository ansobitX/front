...
export class WalletTable extends React.Component<Props> {
    ...
    private formatTxState = (tx: string, confirmations?: number, minConfirmations?: number) => {
        const statusMapping = {
            succeed: <span className="success">{this.translate('page.body.wallets.table.succeed')}</span>,
            failed: <span className="fail">{this.translate('page.body.wallets.table.failed')}</span>,
            accepted: <span className="success">{this.translate('page.body.wallets.table.accepted')}</span>,
            collected: <span className="success">{this.translate('page.body.wallets.table.collected')}</span>,
            canceled: <span className="fail">{this.translate('page.body.wallets.table.canceled')}</span>,
            rejected: <span className="fail">{this.translate('page.body.wallets.table.rejected')}</span>,
            processing: <span className="wait">{this.translate('page.body.wallets.table.pending')}</span>,
            prepared: <span className="wait">{this.translate('page.body.wallets.table.pending')}</span>,
            submitted: (confirmations !== undefined && minConfirmations !== undefined) ? (
                <span className="wait">{`${confirmations}/${minConfirmations}`}</span>
            ) : (
                <span className="wait">{this.translate('page.body.wallets.table.pending')}</span>
            ),
            skipped: <span className="success">{this.translate('page.body.wallets.table.skipped')}</span>,
        };
